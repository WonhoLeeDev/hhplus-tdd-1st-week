...
public class UserPointUseTest {
    ...
    @Test
    void useUserPointUpdateTest() throws InterruptedException {
        ...
        Long useUserPoint = 30L;
        UserPoint usedUserPoint = userPointService.useUserPoint(chargedUserPoint.id(), useUserPoint);
        assertThat(usedUserPoint.point()).isEqualTo(chargePoint - useUserPoint);
    }

    @DisplayName("ID 또는 사용 포인트가 null일 때, 에러가 던져지고 포인트 사용에 실패한다.")
    @Test
    void validateUseUserPointNullArgument() {
        assertThatExceptionOfType(IllegalArgumentException.class)
                .isThrownBy(() -> {
                    userPointService.useUserPoint(null, null);
                }).withMessage("ID 또는 포인트가 null 입니다.");
    }

    @DisplayName("사용할 포인트가 0보다 작으면, 에러가 던져지고 포인트 사용에 실패한다.")
    @Test
    void validateUseUserPointNegativePointArgument() throws InterruptedException {
        UserPoint userPoint = new UserPoint(1L, 50L, System.currentTimeMillis());
        UserPoint chargedUserPoint = userPointService.chargeUserPoint(userPoint.id(), userPoint.point());
    ...
    @Test
    void validateUseUserPointAmountArgument() throws InterruptedException {
        UserPoint userPoint = new UserPoint(1L, 50L, System.currentTimeMillis());
        UserPoint chargedUserPoint = userPointService.chargeUserPoint(userPoint.id(), userPoint.point());

        Long useUserPoint = 51L;
        assertThatExceptionOfType(IllegalArgumentException.class)
                .isThrownBy(() -> {
                    userPointService.useUserPoint(chargedUserPoint.id(), useUserPoint);
